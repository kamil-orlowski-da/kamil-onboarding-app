...
import com.digitalasset.quickstart.api.CarDealersApi;
...
public class CarDealersApiImpl implements CarDealersApi {
    ...
    @Override
    @WithSpan
    public CompletableFuture<ResponseEntity<CarDealer>> createCarDealer(CreateCarDealerRequest request) {
        ...
        return traceServiceCallAsync(ctx, () -> CompletableFuture.supplyAsync(() -> {
            ...
            Actor actor = registry.register(
                    Role.CAR_DEALER, partyName(request.getName()), requestedParty(request.getParty()));
            return ResponseEntity.status(HttpStatus.CREATED).body(new CarDealer(
