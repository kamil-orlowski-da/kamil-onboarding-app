...
final class PartyRequests {
     ...
     *
     * <p>The registry slugs the name into the party id, so a name with nothing sluggable in it
     * slugs to nothing. Rejecting it here gives a 400 rather than {@code CarDealer::unnamed}.
     *
     * <p>ASCII on purpose: this is a demo, and names in other scripts are out of scope. The cost
     * is that "Żółć" is a 400 — widen this and the registry's {@code slug} together, to
     * {@code \p{IsAlphabetic}}/{@code \p{IsDigit}}, if that ever needs to work.
     */
     ...
     *
     * <p>Blank counts as absent rather than as an error: a form that submits an untouched
     * optional field should register a party, not fail. Not otherwise validated — whether a party
     * exists on the ledger is Canton's to answer, and a registry entry for a party that does not
     * exist simply never matches anyone.
     */
