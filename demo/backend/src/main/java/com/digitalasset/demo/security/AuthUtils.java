...
public class AuthUtils {
  ...
  public <T> CompletableFuture<ResponseEntity<T>> asAdminParty(
      Function<String, CompletableFuture<ResponseEntity<T>>> cf) {
    return asAuthenticatedParty(
        party -> {
          if (party.equals(appProviderPartyId)) {
            return cf.apply(party);
          } else {
            logger.error(
                "Access denied: authenticated party '{}' does not match AppProvider party '{}'.",
                party,
                appProviderPartyId);
  ...
  public <T> CompletableFuture<ResponseEntity<T>> asAuthenticatedParty(
      Function<String, CompletableFuture<ResponseEntity<T>>> cf) {
    return authenticatedPartyProvider
        .getParty()
        .map(cf)
        .orElseGet(
            () -> {
              logger.error(
                  "Authentication failed: no authenticated party present in the security context");
              return CompletableFuture.completedFuture(
