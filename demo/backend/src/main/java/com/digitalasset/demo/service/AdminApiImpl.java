...
import com.digitalasset.demo.api.AdminApi;
import com.digitalasset.demo.repository.TenantPropertiesRepository;
import com.digitalasset.demo.security.AuthUtils;
import com.digitalasset.demo.security.oauth2.AuthClientRegistrationRepository;
import com.digitalasset.demo.security.oauth2.AuthClientRegistrationRepository.Client;
...
public class AdminApiImpl implements AdminApi {
  ...
  public AdminApiImpl(
    ...
    if (auth.isOAuth2Enabled() && authClientRegistrationRepository.isEmpty()) {
      throw new IllegalStateException(
          "OAuth2 authentication is enabled but AuthClientRegistrationRepository is not configured");
    } else if (auth.isSharedSecretEnabled() && userDetailsManager.isEmpty()) {
      throw new IllegalStateException(
          "Shared secret authentication is enabled but UserDetailsManager is not configured");
    }
  ...
  private void validateRequest(@NotNull TenantRegistrationRequest request) {
    Function<String, ResponseStatusException> badRequestExc =
        msg -> new ResponseStatusException(HttpStatus.BAD_REQUEST, msg);
  ...
  private void ensureTenantIsUnique(TenantRegistrationRequest request) {
    Function<String, ResponseStatusException> conflictExc =
        msg -> new ResponseStatusException(HttpStatus.CONFLICT, msg);
    if (tenantPropertiesRepository.getTenant(request.getTenantId()) != null) {
      throw conflictExc.apply("TenantId already exists");
    }
    if (auth.isOAuth2Enabled()) {
      boolean clientIssuerCombinationExists =
          authClientRegistrationRepository.getClientRegistrations().stream()
              .anyMatch(
                  c ->
                      c.getClientId().equals(request.getClientId())
                          && c.getIssuerURL().equals(request.getIssuerUrl()));
  ...
  private void persistTenantMetadata(TenantRegistrationRequest request) {
    TenantPropertiesRepository.TenantProperties props =
        new TenantPropertiesRepository.TenantProperties();
  ...
  public CompletableFuture<ResponseEntity<TenantRegistration>> createTenantRegistration(
    ...
    var ctx =
        tracingCtx(
            ...
            "partyId",
            request.getPartyId());
    return auth.asAdminParty(
        party ->
            traceServiceCallAsync(
                ctx,
                () ->
                    ...
                    CompletableFuture.supplyAsync(
                        () -> {
                          ...
                          if (auth.isOAuth2Enabled()) {
                                ...
                                authClientRegistrationRepository.getClientRegistrations().stream()
                                    .map(
                                        c -> {
                                          ...
                                          TenantPropertiesRepository.TenantProperties props =
                                              tenantPropertiesRepository.getTenant(c.getTenantId());
                                          if (props != null) {
                            ...
                            result =
                                tenantPropertiesRepository.getAllTenants().values().stream()
                                    .map(
                                        prop -> {
                                          TenantRegistration out = new TenantRegistration();
