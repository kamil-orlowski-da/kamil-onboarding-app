...
public class OAuth2ClientRegistrationRepository
    implements AuthClientRegistrationRepository,
  ...
  public OAuth2ClientRegistrationRepository(OAuth2ClientProperties properties) {
    ...
    List<ClientRegistration> baseRegistrations =
        new ArrayList<>(
            new OAuth2ClientPropertiesMapper(properties).asClientRegistrations().values());

    // Build up the map
    this.registrations =
        baseRegistrations.stream()
            .collect(
                Collectors.toMap(
                    ClientRegistration::getRegistrationId, registration -> registration));
  }
  ...
  public String registerClient(Client client) throws IllegalArgumentException {
    ...
    if (exists) {
      throw new IllegalArgumentException(
          "Duplicate client registration not allowed for clientId="
              + client.getClientId()
              + " and issuerURL="
              + client.getIssuerURL());
    }
  ...
  public void removeClientRegistration(String tenantId, String clientId) {
    ...
    if (registrations.remove(key) == null) {
      throw new NoSuchElementException(
          "No registration found for tenantId=" + tenantId + " clientId=" + clientId);
    }
  }
  ...
  public Collection<Client> getClientRegistrations() {
    return registrations.values().stream()
        .filter(
            r -> r.getAuthorizationGrantType().equals(AuthorizationGrantType.AUTHORIZATION_CODE))
        .map(
            r -> {
              Client c = new Client();
