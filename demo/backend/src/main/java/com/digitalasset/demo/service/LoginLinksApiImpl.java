...
import com.digitalasset.demo.api.LoginLinksApi;
...
public class LoginLinksApiImpl implements LoginLinksApi {
  ...
  public CompletableFuture<ResponseEntity<List<LoginLink>>> listLinks() {
    return CompletableFuture.supplyAsync(
        () -> {
          List<LoginLink> links =
              clientRegistrationRepository.getClientRegistrations().stream()
                  .map(
                      registration ->
                          new LoginLink()
                              .name(registration.getTenantId())
                              .url(
                                  clientRegistrationRepository.getLoginLink(
