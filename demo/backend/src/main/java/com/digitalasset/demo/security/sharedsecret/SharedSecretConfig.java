...
package com.digitalasset.demo.security.sharedsecret;

import com.digitalasset.demo.config.SecurityConfig;
import com.digitalasset.demo.repository.TenantPropertiesRepository;
import com.digitalasset.demo.security.Auth;
import com.digitalasset.demo.security.AuthenticatedPartyProvider;
import com.digitalasset.demo.security.AuthenticatedUserProvider;
import com.digitalasset.demo.security.TokenProvider;
...
public class SharedSecretConfig {
  ...
  public SharedSecretConfig(
      TenantPropertiesRepository tenantPropertiesRepository, SecurityConfig securityConfig) {
              ...
              props
                  ...
                  .forEach(
                      userId -> {
                        ...
                        if (props.isInternal()) userBuilder.roles("ADMIN");
  ...
  public AuthenticatedUserProvider authenticatedUserProvider() {
    return () -> {
      if (SecurityContextHolder.getContext().getAuthentication()
        ...
        return tenantPropertiesRepository.getAllTenants().values().stream()
            .filter(tenant -> tenant.getUsers().contains(name))
            .findFirst()
            .map(
                tenant -> {
                  List<String> roles;
                  ...
                  return new AuthenticatedUserProvider.DefaultAuthenticatedUser(
                      userName, tenant.getTenantId(), tenant.getPartyId(), roles, admin);
                });
      }
    };
  }
