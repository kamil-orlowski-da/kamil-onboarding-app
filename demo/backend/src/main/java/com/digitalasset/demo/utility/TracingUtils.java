...
package com.digitalasset.demo.utility;
...
/**
 * The {@code TracingUtils} is a utility class designed to centralize common logging and
 * OpenTelemetry trace operations. Its primary purpose is to reduce repetitive code when adding the
 * same structured attributes to both log statements and the current OpenTelemetry {@link Span}.
 *
 * <p>This helps ensure consistency between logs and tracing data, making it easier to correlate log
 * entries with specific span attributes and events. By standardizing attribute handling, you also
 * avoid the risk of typos or mismatched field names in separate logging vs. tracing code paths.
 *
 * <p>Typical usage includes:
 *
 * <ul>
 *   <li>Setting top-level attributes on a span via {@link #setSpanAttributes(Span, Map)}.
 *   <li>Adding events (with attributes) to a span via {@link #addEventWithAttributes(Span, String,
 *       Map)}.
 ...
 *
 * <ol>
 *   <li>Log key contextual data (e.g., commandId, contractId) as structured fields.
 *   <li>Set or record the same data on the current span for distributed tracing.
 *   <li>Maintain consistency across your logs and telemetry.
 * </ol>
 *
 * <p>If your code does not require adding attributes to spans or correlating them with logs, you
 * can bypass this helper and use standard logging calls directly.
 */
public final class TracingUtils {
  ...
  /**
   * Add attributes to the current Span from a map. If attributes is null, does nothing.
   *
  ...
  /**
   * Add an event to the span with optional attributes. If attributes is null, just add the event
   * name with no extra attributes.
   *
   * @param span the current OpenTelemetry Span; may be null
   * @param eventName the name of the event
   * @param attributes the map of key-value attributes to attach to the event; may be null
   */
  public static void addEventWithAttributes(
      Span span, String eventName, Map<String, Object> attributes) {
    if (span == null) {
      return;
    }
    if (attributes == null) {
  ...
  public static void logError(
      Logger logger, String message, Map<String, Object> attributes, Throwable t) {
    if (logger == null) {
      return;
    }
  ...
  /**
   * Create a TracingContext with a message and optional key-value attribute pairs. The args must
   * contain an odd number of elements: the first is the message (String), followed by pairs of key
  ...
  public static TracingContext tracingCtx(@NotNull Logger logger, Object... args) {
    if (args == null || args.length % 2 == 0) {
      throw new IllegalArgumentException(
          "attrs requires an odd number of arguments message plus multiple key, value pairs.");
    }
  ...
  public static <T> CompletableFuture<T> traceWithStartEvent(
      TracingUtils.TracingContext ctx, Supplier<CompletableFuture<T>> body) {
    return _trace(ctx, true, body);
  }

  public static <T> CompletableFuture<T> traceWithStartEventAsync(
      TracingUtils.TracingContext ctx, Supplier<CompletableFuture<T>> body) {
    return CompletableFuture.supplyAsync(() -> _trace(ctx, true, body)).thenCompose(f -> f);
  }
  ...
  public static <T> CompletableFuture<T> traceAsync(
      TracingUtils.TracingContext ctx, Supplier<CompletableFuture<T>> body) {
    return CompletableFuture.supplyAsync(() -> _trace(ctx, false, body)).thenCompose(f -> f);
  }

  public static <T> CompletableFuture<T> runAndTraceAsync(
      TracingUtils.TracingContext ctx, Supplier<T> body) {
  ...
  private static <T> CompletableFuture<T> _trace(
      TracingUtils.TracingContext ctx, boolean startEvent, Supplier<CompletableFuture<T>> body) {
    // Note: Capturing span here works well, relying on OT CompletableFuture instrumentation
    // If we later need more control over context propagation we can always capture
    ...
    return body.get()
        .whenComplete(
            (res, ex) -> {
              if (ex != null) {
                var e =
                    ex instanceof CompletionException ce && ce.getCause() != null
                        ? ce.getCause()
                        : ex;
